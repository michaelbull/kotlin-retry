 ...
 */
fun <E> stopAtAttempts(count: Int): RetryPolicy<E> {
 ...
 * ```
 *
 * @throws [IllegalArgumentException] if [count] is non-negative.
 */
fun <E> stopAtRetries(count: Int): RetryPolicy<E> {
 ...
 */
fun <E> RetryPolicy<E>.stopAtDelay(delayMillis: Long): RetryPolicy<E> {
 ...
 */
fun <E> RetryPolicy<E>.stopAtCumulativeDelay(delayMillis: Long): RetryPolicy<E> {
    require(delayMillis > 0) { "delayMillis must be positive, but was: $delayMillis" }
